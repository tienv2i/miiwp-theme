...
if (!function_exists('miiwp_widgets_init')) :
  ...
  function miiwp_widgets_init() {
    ...
    register_sidebar(array(
      ...
      'before_widget' => '<section id="%1$s" class="widget %2$s card card-body border-0">',
      'after_widget'  => '</section>',
      'before_title'  => '<h2 class="widget-title card-title border-bottom">',
      'after_title'   => '</h2>',
    ));
    // Sidebar End
...
function bootscore_scripts() {
  ...
  if (file_exists(get_template_directory() . '/assets/dist/bundle.css')) {
    $modificated_bootscoreCss = date('YmdHi', filemtime(get_template_directory() . '/assets/dist/bundle.css'));
  } else {
    $modificated_bootscoreCss = 1;
  }
  $modificated_themeJs = date('YmdHi', filemtime(get_template_directory() . '/assets/dist/bundle.js'));

  // Style CSS
  wp_enqueue_style('bootscore-style', get_stylesheet_uri(), array(), $modificated_styleCss);

  // bootScore
  wp_enqueue_style('main', get_template_directory_uri() . '/assets/dist/bundle.css', array(), $modificated_bootscoreCss);

  // Theme JS
  wp_enqueue_script('bootscore-script', get_template_directory_uri() . '/assets/dist/bundle.js', array('jquery'), $modificated_themeJs, true);
